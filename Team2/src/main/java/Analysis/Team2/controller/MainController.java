...
public class MainController {
    ...
    public CompletableFuture<ResponseEntity<String>> mainRequest(@RequestBody String requestBody) {
        ...
        return codeFuture.thenCompose(code -> {
            // Call to all other async services and wait for their completion
...
//        inputContent.append("예상 매출액 : 500000000원. ");
        ...
        inputContent.append("대답은 미사여구를 붙이지말고, 업종의 전체 월 매출 합산 예상 데이터, 업종의 일별 매출액 추이 예상 데이터, 주고객 연령 적합도 각각에 대한 분석을 바탕으로 창업에 대해 부정적인지, 보통인지, 긍정적인지를 신호등색깔(적색,황색,녹색)로 각각에 대해 판단해서 답변해줘. 현재 날짜와 대비해서 미래에 어떤 예상을 보이는지 엄격하게 살펴봐야해.");
